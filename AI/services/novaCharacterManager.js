...
import { novaAccountScope } from './novaAccountScope.js';
...
class NovaCharacterManager extends EventTarget {
  constructor(){
    super();
    let saved='nova';
    try{saved=localStorage.getItem(novaAccountScope.key(STORAGE_KEY))||'nova'}catch(_){}
    this.current=allowed.has(saved)?saved:'nova';
    novaAccountScope.subscribe(()=>this.reloadForAccount());
  }
  ...
  setCharacter(id){
    ...
    this.current=next;
    try{localStorage.setItem(novaAccountScope.key(STORAGE_KEY),next)}catch(_){}
  ...
  reloadForAccount(){
    let saved='nova';
    try{saved=localStorage.getItem(novaAccountScope.key(STORAGE_KEY))||'nova'}catch(_){}
    const next=allowed.has(saved)?saved:'nova';
    if(next===this.current)return;
    this.current=next;
    const detail={id:next,character:this.getDefinition(next),accountChanged:true};
    this.dispatchEvent(new CustomEvent('change',{detail}));
    window.dispatchEvent(new CustomEvent('nova:character-change',{detail}));
  }
  subscribe(listener){
