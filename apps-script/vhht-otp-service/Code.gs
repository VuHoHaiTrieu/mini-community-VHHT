...
function doPost(event) {
  try {
    ...
    return json_({
      ok: false,
      code: 'SERVER_ERROR',
      message: 'Dịch vụ OTP đang bận. Vui lòng thử lại.'
    });
  }
}
...
function requestOtp_(payload) {
  ...
  try {
    ...
    if (
      existing &&
      now - Number(existing.lastSent || 0) < CONFIG.RESEND_COOLDOWN_MS
    ) {
      return json_({
        ok: false,
        delivered: false,
        ...
        retryAfter: Math.ceil(
          (
            CONFIG.RESEND_COOLDOWN_MS -
            (now - Number(existing.lastSent || 0))
          ) / 1000
        ),
    ...
    const windowStartedAt =
      existing &&
      now - Number(existing.windowStartedAt || 0) < 3600000
        ? Number(existing.windowStartedAt)
        : now;

    const sendCount =
      existing && windowStartedAt === Number(existing.windowStartedAt)
        ? Number(existing.sendCount || 0)
        : 0;

    if (sendCount >= CONFIG.MAX_SENDS_PER_HOUR) {
      return json_({
        ok: false,
        delivered: false,
        code: 'RATE_LIMITED',
    ...
    if (!account) {
      console.warn(
        '[OTP] Không tìm thấy tài khoản Firebase cho email hash: ' +
    ...
    console.log(
      '[OTP] Đã gửi mã tới tài khoản Firebase uid: ' +
      account.localId
    );
...
function verifyOtp_(payload) {
  ...
  if (!validEmail_(email) || !/^\d{6}$/.test(otp)) {
    return json_({
      ok: false,
      code: 'INVALID_OTP',
  ...
  try {
    ...
    if (Number(record.attempts || 0) >= CONFIG.MAX_ATTEMPTS) {
      ...
      return json_({
        ok: false,
        code: 'OTP_LOCKED',
        message: 'Bạn đã nhập sai quá nhiều lần. Hãy yêu cầu mã mới.'
      });
    }

    const submittedHash = digest_(record.salt + ':' + otp);

    if (!safeEqual_(record.otpHash, submittedHash)) {
      ...
      return json_({
        ...
        code: 'INVALID_OTP',
        remainingAttempts:
          CONFIG.MAX_ATTEMPTS - record.attempts,
    ...
    properties.setProperty(
      'reset_' + digest_(resetToken),
      JSON.stringify({
        uid: record.uid,
        email: email,
        expiresAt: Date.now() + CONFIG.RESET_TTL_MS
      })
    );
    ...
    return json_({
      ...
      expiresIn: Math.floor(CONFIG.RESET_TTL_MS / 1000)
    });
  } finally {
    lock.releaseLock();
  }
...
function resetPassword_(payload) {
  ...
  if (resetToken.length < 40 || password.length < 8) {
    return json_({
      ok: false,
      code: 'INVALID_RESET',
...
function lookupAccount_(email) {
  const result = firebaseRequest_(
    'accounts:lookup',
    { email: [email] }
  );

  return result.users && result.users.length
    ? result.users[0]
    : null;
}
...
function firebaseRequest_(method, body) {
  ...
  const result =
    parse_(response.getContentText()) || {};
  ...
  if (
    ...
    throw new Error(
      ...
      'Firebase Authentication API error'
    );
  }
    ...
    htmlBody:
      ...
      'color:#eaf2ff">' +
        ...
        'text-align:center">' +
          ...
          '<img src="' +
          CONFIG.LOGO_URL +
          '" alt="VHHT" style="max-width:210px;height:auto">' +

          '<h2 style="margin:24px 0 8px">' +
          'Xác thực đặt lại mật khẩu' +
          '</h2>' +

          '<p style="color:#b9c8df">' +
          'Nhập mã dưới đây trên website VHHT:' +
          '</p>' +
