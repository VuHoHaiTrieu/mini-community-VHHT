...
function syncMobileComposerLayout(){const form=$("message-form"),input=$("message-input"),hasText=Boolean(input.value.trim()),engaged=hasText||document.activeElement===input;form.classList.toggle("composer-input-expanded",engaged);if(!engaged)form.classList.remove("composer-tools-revealed");const collapsed=engaged&&!form.classList.contains("composer-tools-revealed");const button=$("composer-more-button");button.innerHTML=`<i class="fa-solid ${collapsed?'fa-chevron-right':'fa-plus'}" aria-hidden="true"></i>`;button.title=collapsed?'Mở lại công cụ':'Mở thêm công cụ';button.setAttribute('aria-label',button.title)}
