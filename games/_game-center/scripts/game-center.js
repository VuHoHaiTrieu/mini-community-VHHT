...
if (canvas && context) {
  const compact = matchMedia('(max-width:700px),(pointer:coarse)').matches, phonePowerMode = matchMedia('(max-width:900px) and (pointer:coarse)').matches, reducedMotion = matchMedia('(prefers-reduced-motion:reduce)').matches;
  let stars = [], meteors = [], lastFrame = 0, meteorTimer = 3200, frame = 0;
  const createStars = () => { const count = Math.min(phonePowerMode ? 76 : 230, Math.max(phonePowerMode ? 50 : 150, Math.round(innerWidth * innerHeight / (phonePowerMode ? 8500 : 5200)))); stars = Array.from({ length: count }, () => { const distance = Math.random(); return { x: Math.random() * innerWidth, y: Math.random() * innerHeight, size: distance < .72 ? .35 + Math.random() * .75 : .9 + Math.random() * 1.35, alpha: .24 + Math.random() * .7, speed: .0015 + Math.random() * .0035, drift: .006 + Math.random() * .022, tint: Math.random() > .84 ? (Math.random() > .5 ? '174, 224, 255' : '205, 190, 255') : '255, 255, 255', diamond: Math.random() > .78 }; }); };
  ...
  const draw = time => { frame = 0; if (document.hidden) return; if (time - lastFrame < (reducedMotion ? 160 : phonePowerMode ? 84 : 26)) { frame = requestAnimationFrame(draw); return; } lastFrame = time; context.clearRect(0, 0, innerWidth, innerHeight);
    ...
    if (!reducedMotion && !phonePowerMode && time > meteorTimer) { shootingStar(); meteorTimer = time + 5500 + Math.random() * 6500; }
