...
export class GameAudio {
  ...
  setMusic(name,{level=.82,fade=1}={}) { if(this.music?.name===name){this.music.level=level;this.applyVolumes();if(this.music.audio.paused&&!document.hidden)this.music.audio.play().catch(()=>{});return;}const previous=this.music,newAudio=new Audio(this.url(name));newAudio.loop=true;newAudio.preload='auto';newAudio.volume=.0001;this.music={name,audio:newAudio,level};if(!document.hidden)newAudio.play().catch(()=>{});const started=performance.now(),target=this.musicVolume(level);const crossfade=()=>{if(document.hidden){newAudio.pause();if(previous)previous.audio.pause();return}const t=Math.min(1,(performance.now()-started)/(fade*1000));newAudio.volume=Math.max(.0001,target*t);if(previous)previous.audio.volume=Math.max(.0001,this.musicVolume(previous.level)*(1-t));if(t<1)requestAnimationFrame(crossfade);else if(previous){previous.audio.pause();previous.audio.currentTime=0;}};requestAnimationFrame(crossfade); }
  stopMusic(fade=.45) { const current=this.music;if(!current)return;this.music=null;const started=performance.now(),initial=current.audio.volume;const fadeOut=()=>{if(document.hidden){current.audio.pause();current.audio.currentTime=0;return}const t=Math.min(1,(performance.now()-started)/(fade*1000));current.audio.volume=Math.max(.0001,initial*(1-t));if(t<1)requestAnimationFrame(fadeOut);else{current.audio.pause();current.audio.currentTime=0;}};requestAnimationFrame(fadeOut); }
  ...
  pauseAll(){this.clearIntro();this.music?.audio.pause();this.loops.forEach(audio=>audio.pause());this.active.forEach(audio=>audio.pause());this.active.clear();}
