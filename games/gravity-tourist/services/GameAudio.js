...
export class GameAudio {
  constructor() {
    ...
    window.addEventListener('vhht:sound-settings-change', event => { this.settings = { ...this.settings, ...(event.detail || {}) }; this.applyVolumes(); });
    document.addEventListener('visibilitychange', () => { if (document.hidden) this.pauseAll(); else this.resumeMusic(); });
  }
  readSettings() { try { return { muted:false,effectsEnabled:true,musicEnabled:true,masterVolume:.72,effectsVolume:.62,musicVolume:.34,...JSON.parse(localStorage.getItem(SETTINGS_KEY)||'{}') }; } catch { return { muted:false,effectsEnabled:true,musicEnabled:true,masterVolume:.72,effectsVolume:.62,musicVolume:.34 }; } }
  url(name) { return new URL(`../assets/audio/${CUE_FOLDERS[name]}/${name}.${this.extension}`, import.meta.url).href; }
  effectVolume(level=1) { return this.settings.muted || !this.settings.effectsEnabled ? 0 : clamp(this.settings.masterVolume) * clamp(this.settings.effectsVolume) * level; }
  musicVolume(level=1) { return this.settings.muted || !this.settings.musicEnabled ? 0 : clamp(this.settings.masterVolume) * clamp(this.settings.musicVolume) * level; }
  ...
  resumeMusic(){if(this.music&&!this.settings.muted&&this.settings.musicEnabled)this.music.audio.play().catch(()=>{});this.loops.forEach(audio=>audio.play().catch(()=>{}));}
  applyVolumes(){if(this.music)this.music.audio.volume=this.musicVolume(this.music.level);for(const [name,audio]of this.loops)audio.volume=this.effectVolume(name==='orbit-loop'?.22:.25);if(this.settings.muted){this.active.forEach(audio=>audio.pause());}}
  toggleMute(){this.settings.muted=!this.settings.muted;try{const stored=JSON.parse(localStorage.getItem(SETTINGS_KEY)||'{}');localStorage.setItem(SETTINGS_KEY,JSON.stringify({...stored,muted:this.settings.muted}));}catch{}this.applyVolumes();window.dispatchEvent(new CustomEvent('vhht:sound-settings-change',{detail:{muted:this.settings.muted}}));if(!this.settings.muted)this.resumeMusic();return this.settings.muted;}
