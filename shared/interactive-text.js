...
const TOKEN_PATTERN = /(https?:\/\/[^\s<]+|www\.[^\s<]+|@[A-Za-z0-9._]{2,32}\b|(?<!\d)0(?:[\s.-]?\d){8,10}(?!\d))/g;
...
export function renderInteractiveText(value) {
    ...
    while ((match = TOKEN_PATTERN.exec(source))) {
        ...
        if (/^(?:https?:\/\/|www\.)/i.test(token)) {
            ...
            const label = token.startsWith("@") ? "ID" : "số điện thoại";
            output += `<button type="button" class="interactive-copy-token" data-interactive-copy="${escapeInteractiveHtml(copyValue)}" aria-label="Sao chép ${label}" title="Sao chép ${label}">${escapeInteractiveHtml(token)} <i class="fa-regular fa-copy" aria-hidden="true"></i></button>`;
        }
